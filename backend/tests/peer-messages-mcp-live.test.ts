...
test('MCP audit wrapper preserves real initialize and tool discovery without model or credentials',async()=>{
  ...
  try{
    ...
    assert.deepEqual((await response(2)).result.tools.map((x:any)=>x.name).sort(),['agent_context','agent_inbox','agent_outbox','agent_send']);
    const audit=await readMcpAudit(dir);assert.ok(audit.some(x=>x.phase==='reply'&&x.method==='tools/list'&&x.ok&&x.tools.length===4));
