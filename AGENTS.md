...
**如果两边都是 Claude Code，用它自带的会话间通信，不要用这一套。** 那个不需要凭证、不需要
钉子、不需要知道对方的 conversationId，而且本来就在。绕到这里来只是把简单的事做复杂。

这一套的存在理由只有两条：

- **跨 CLI**。claude ↔ qwen ↔ codex 之间说话，自带的那套办不到，因为它只认自己。
- **人在环里**。消息进库、在网页上看得见、能取消、卡住了有写明原因。自带的那套是两个进程
  之间的事，用的人看不见也插不上手。

**这两条都用不上的时候，这一套就是绕远路。** 尤其注意第一条今天是有边界的：只有能接受
命令提交的 CLI 才收得到（`peers` 会如实标出来），omp 这类没有提交通道的会一直停在
`queued`——消息不会丢，但也不会到。
