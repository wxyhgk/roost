...
export const misc = {
  ...
  store: {
    ...
    pinAppFailed: "Could not pin the app, try again",
  ...
  conversations: {
    ...
    detail: {
      ...
      live: {
        ...
        waitingPermission: "Waiting for you to approve something",
      ...
      send: {
        ...
        blocked: {
          ...
          unboundHint: "The binding still points at this terminal's previous process. Typing in the terminal will not recover it — use Rebind to move it over.",
          rebind: "Rebind",
          rebinding: "Rebinding…",
          rebindHint: "Move the binding to this terminal's current process. Identity still comes from what the CLI itself reported; nothing changes if it cannot be confirmed.",
          rebindUnconfirmed: "Could not confirm the conversation identity from this terminal's records; the binding is unchanged",
          rebindConflict: "This conversation is already bound to another terminal",
        ...
        queuedReason: {
          ...
          foregroundNotCli: "Another program is in the foreground of that terminal",
          foregroundNotCliHint: "The CLI may have opened an editor or pager (the vim that git commit starts, say). This sends itself once the CLI is back in front.",
          foregroundUnknown: "Cannot tell what is in the terminal's foreground, so nothing is written",
          ...
          acceptanceUncertain: "An earlier message is still unconfirmed",
          acceptanceUncertainHint: "The previous one was written to the terminal but the CLI has not confirmed it, and everything after it waits while it is unresolved. If it was never actually sent, the confirmation will never come — choose “Not sent — drop it” on that one and this one goes next.",
          awaitingPasteEcho: "Pasted; waiting for the terminal to echo it back",
          acceptanceTimeout: "Written, but the CLI never confirmed",
          acceptanceTimeoutHint: "It was most likely submitted. This is **not** resent automatically — resending could submit the same message twice. Check the terminal.",
          targetChanged: "The target terminal changed before the write",
          writeBoundaryUnknown: "The write lost its boundary midway",
          writeBoundaryUnknownHint: "How much got written is unknown, so this is kept and never resent automatically.",
          writeFailed: "Writing to the terminal failed",
          recipientBlocked: "An earlier message is stuck and holding up the rest",
          ...
          submissionBoundaryUnknown: "Interrupted mid-submission; unclear whether it was written",
          commandEvidenceMismatch: "The evidence does not match; cannot confirm where this went",
        ...
        direct: {
          ...
          pasted: "Pasted into the terminal's input box, but we never saw it appear, so we did not press Enter for you. Check the terminal and press it yourself if it looks right.",
          held: {
            dialog: "A picker is open in the terminal — pressing Enter now would choose for you, so nothing was written. Deal with it in the terminal first, then send.",
            not_cli: "The terminal's foreground is not an AI CLI (probably a shell), so nothing was sent — typing into a shell and pressing Enter runs a command.",
            foreground_unknown: "Cannot tell what is in the terminal's foreground right now, so nothing was sent. Try again shortly.",
            screen_unavailable: "Cannot see the terminal screen right now, so nothing was sent. Try again shortly.",
            no_input_box: "Cannot find the CLI's input box on the terminal screen, so nothing was sent. Check what it is doing.",
          } as Record<string, string>,
          heldOther: "Not sent. Check the terminal.",
          staleHistory: "The history shown here may not be the session running in the terminal. Messages still go into the terminal — they will just show up in that session.",
        },
        ...
        uncertainHint: "The request is kept and will not resend automatically — resending could submit the same text twice. While it is unresolved, later messages to this conversation wait for it.",
        failed: "Delivery failed",
        cancelled: "Cancelled",
        dismissed: "Dropped",
        dismiss: "Not sent — drop it",
        dismissConfirm: "Confirm this never reached the CLI?\n\nOnce dropped, the messages queued behind it can go.\n\nIf it is still sitting in the terminal input box, dropping does not remove it — pressing Enter there will still send it.",
