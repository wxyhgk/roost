...
export const misc = {
      ...
      send: {
        ...
        queuedReason: {
          ...
          */
          ...
          acceptanceUncertainHint: "上一条已经写进终端，但还没等到 CLI 的确认，它悬着的时候后面的都会等它。它要是其实没发出去，就永远等不到确认——那时在那一条上点「没发出去，放弃」，这条就会接着发。",
          ...
          writeFailed: "往终端写入失败",
          recipientBlocked: "前面有一条没处理完，卡住了后面的",
        ...
        */
        /*
          直接往终端里打字的结果（见 terminal-daemon/src/direct-input.ts）。每一句只说这一句，
          不会因为上一句没发成而挡住下一句。
        */
        direct: {
          submitted: "已发到终端",
          pasted: "已经贴进终端的输入框，但没在屏幕上看到它，所以没替你按回车。去终端看一眼，没问题就自己按一下。",
          held: {
            dialog: "终端里正开着一个选择框，这时按回车就等于替你选了，所以一个字都没写。先去终端处理它，再发。",
            not_cli: "终端前台现在不是 AI CLI（多半是 shell），没有发——往 shell 里打字再回车就是执行一条命令。",
            foreground_unknown: "现在判断不了终端前台是谁，没有发。稍后再试。",
            screen_unavailable: "现在看不到终端画面，没有发。稍后再试。",
            no_input_box: "终端画面上找不到 CLI 的输入框，没有发。去终端看一眼它在干什么。",
          } as Record<string, string>,
          heldOther: "没有发出去。去终端看一眼。",
          staleHistory: "这里显示的对话记录可能不是终端里正在跑的那一段。发出去的话照样进终端，只是会出现在那一段里。",
        },
        /*
        ...
        */
        ...
        awaitingUserSubmitHint: "正文已经放进输入框了，但我们没在屏幕上等到它出现，所以没替你按那一下回车——没看见落点就不按。你按完这条会自己标成已送达。",
        uncertain: "可能已写入，但没有拿到确认",
        uncertainHint: "保留这条请求，不会自动重发——重发可能造成同一句话提交两次。它悬着的时候，发给这个对话的后续消息都会等它。",
        failed: "投递失败",
        cancelled: "已取消",
        dismissed: "已放弃",
        dismiss: "没发出去，放弃",
        dismissConfirm: "确认这条没有发到 CLI 里？\n\n放弃之后，排在它后面的消息才能接着发。\n\n它若还留在终端输入框里，放弃不会把它删掉——你在终端里按回车，它照样会发出去。",
        goTerminal: "去终端处理",
      },
    },
  },
  ...
  launchpad: {
    title: "应用",
    hint: "本机在跑的服务。点一下在新标签页打开",
    empty: "没有可打开的服务",
    unsupported: "这台机器上看不到端口（没有 lsof，或被策略挡住）",
    loading: "正在查看…",
    failed: "取不到服务列表",
    refresh: "重新查看",
    openTab: "新标签页",
    ...
    unpin: "取消固定",
    notRunning: "未运行",
    alsoOn: (ports: string) => `同一进程还占着 ${ports}`,
    /* 同源 iframe 不是隔离，说清楚再让人选，别让「开成窗口」显得比它实际更安全。 */
    windowWarning: "窗口里的应用和 roost 同源，能读到你的 roost 会话。自己起的服务再用",
  },
  windows: {
    openAsWindow: "在窗口中打开",
    close: "关闭窗口",
