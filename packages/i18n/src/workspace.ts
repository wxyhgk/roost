...
export const workspace = {
  ...
  session: {
    ...
    /** 传输连不上后端，与「shell 已退出」是两回事。 */
      offline: "连不上后端",
      /*
        **这句话必须和 connection.ts 里的行为一致。** 超过重试上限之后它并没有停，
        而是转成每 30 秒一次的慢速探针（那段代码有注释说明是故意的）。原来这里写的是
        「自动重连已停止」，于是最常见的那条路上，界面在劝人去查后端——其实它自己会好。
      */
      offlineHint: "正在每 30 秒后台重试。后端起来会自己接上。",
      /** 真的不再重连的那一类（画面过大、replay 不可用）。它和上面是相反的状态。 */
      screenUnavailable: "画面暂时恢复不了",
      retry: "立即重试",
