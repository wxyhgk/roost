...
// 原来输入框 14、包着它的 <label> 12、按钮 12——同一张表单里三档。
// 跟着 SettingsDialog 收敛：控件与字段标签 text-body、说明与状态 text-caption。
const control = 'w-full rounded-md border border-border bg-bg px-3 py-2 text-body text-text outline-none focus-visible:ring-2 focus-visible:ring-accent';
const button = 'rounded-md border border-border px-3 py-2 text-body hover:bg-bg-hover focus-visible:outline-2 focus-visible:outline-accent disabled:cursor-not-allowed disabled:opacity-40';
...
export function CliSettings({ onDirtyChange, onBusyChange }: { onDirtyChange(value: boolean): void; onBusyChange(value: boolean): void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-4">
    <div className="flex shrink-0 items-center justify-between gap-3"><p className="text-caption leading-relaxed text-text-dim">{t.settings.cli.headerHint}</p><button type="button" className={`${button} flex shrink-0 items-center gap-1`} disabled={busy} onClick={() => choose(null)}><PlusIcon className="size-4" />{t.settings.cli.create}</button></div>
    {api.error && <div role="alert" className="rounded-md border border-border p-3 text-caption">{api.error}<button type="button" className={`${button} ml-2`} onClick={() => { void api.refresh().catch(() => {}); }}>{t.settings.cli.retry}</button></div>}
    {api.loading && <p role="status" className="text-caption text-text-dim">{t.settings.cli.loading}</p>}
    ...
    <div className="flex flex-col gap-2 md:min-h-0 md:overflow-y-auto md:pr-2" aria-label={t.settings.cli.listLabel}>
      {api.configs.map(config => <button key={config.id} type="button" disabled={busy} aria-pressed={editing && selected?.id === config.id} onClick={() => choose(config)} className={`${button} flex min-h-12 w-full min-w-0 shrink-0 items-center gap-3 text-left ${editing && selected?.id === config.id ? 'bg-bg-active' : ''}`}>
    ...
    {!editing && !api.loading && <p className="text-caption text-text-dim">{t.settings.cli.empty}</p>}
    {editing && <form className="space-y-4" onSubmit={event => { event.preventDefault(); void run(async () => { await save(); setMessage(t.settings.cli.saved); }); }}>
      {remoteChanged && !busy && <p className="text-caption text-text-dim">{t.settings.cli.serverChanged}</p>}
      <fieldset disabled={busy} className="min-w-0 space-y-4 disabled:opacity-60">
        ...
        <p className="text-caption text-text-dim">{t.settings.cli.logo.hint}</p>
        <label className="block space-y-1 text-body"><span>{t.settings.cli.form.name}</span><input required className={control} value={draft.name} onChange={event => change({ name: event.target.value })} /></label>
        <label className="block space-y-1 text-body"><span>{t.settings.cli.form.command}</span><input required className={control} value={draft.command} placeholder={t.settings.cli.form.commandPlaceholder} onChange={event => change({ command: event.target.value })} /></label>
        <div className="flex flex-wrap items-center gap-4"><label className="flex items-center gap-2 text-body"><input type="checkbox" className="size-4 accent-accent" checked={draft.enabled} onChange={event => change({ enabled: event.target.checked })} />{t.settings.cli.form.enabled}</label><label className="flex items-center gap-2 text-body">{t.settings.cli.form.priority}<input type="number" min={-1000} max={1000} step={1} required className={`${control} max-w-24`} value={draft.priority} onChange={event => change({ priority: event.target.value })} /></label></div>
        <div className="space-y-2"><div className="flex items-center justify-between"><span className="text-body">{t.settings.cli.rules.title}</span><button type="button" className={button} disabled={draft.rules.length >= 32} onClick={() => change({ rules: [...draft.rules, { kind: 'executable', value: '' }] })}>{t.settings.cli.rules.add}</button></div>
          ...
          <p className="text-caption text-text-dim">{t.settings.cli.rules.hint}</p>
        </div>
        <p className="text-caption text-text-dim">{t.settings.cli.capabilities.text}{selected?.capabilities.image ? t.settings.cli.capabilities.withImage : t.settings.cli.capabilities.noImage}{selected && <span className="mt-1 block break-all">{t.settings.cli.capabilities.id(selected.id)}</span>}</p>
      ...
      {dirty && !busy && <p className="text-caption text-text-dim">{t.settings.cli.dirty}</p>}
    </form>}
    {busy && <p role="status" className="text-caption text-text-dim">{t.settings.cli.savingStatus}</p>}
    {error && <p role="alert" className="text-caption text-red-500">{t.settings.cli.errorSuffix(error)}</p>}
    {message && <p role="status" className="text-caption text-text-dim">{message}</p>}
    </div>
    </div>
  </div>;
}
