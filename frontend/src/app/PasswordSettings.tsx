...
export function PasswordSettings({ onBusyChange }: { onBusyChange(busy: boolean): void }) {
  ...
  // 见 SettingsDialog 顶上那段：控件/标签 text-body、说明与状态 text-caption。
  if (loadFailed) return <div className="space-y-3 text-body"><p role="alert">{m.loadFailed}</p><button type="button" onClick={() => setAttempt(value => value + 1)} className="rounded-md border border-border px-3 py-2">{m.retry}</button></div>;
  if (available === null) return <p role="status" className="text-caption text-text-dim">{m.loading}</p>;
  if (!available) return <p role="status" className="text-caption text-text-dim">{m.unavailable}</p>;
  const control = 'w-full rounded-md border border-border bg-bg px-3 py-2 text-body text-text outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:opacity-60';
  const fields = [
  ...
  return <form noValidate onSubmit={event => { event.preventDefault(); void submit(); }} className="max-w-md space-y-4" aria-label={m.title}>
    <p className="text-caption leading-relaxed text-text-dim">{m.hint}</p>
    {fields.map(({ label, value, update, autoComplete, maxLength }) =>
      <label key={label} className="flex flex-col gap-2 text-body"><span>{label}</span><input type={visible ? 'text' : 'password'} value={value} required disabled={busy}
    ...
    <div className="flex justify-end"><button type="button" aria-pressed={visible} disabled={busy} onClick={() => setVisible(!visible)} className="flex shrink-0 items-center gap-1.5 rounded p-1 text-body text-text-dim hover:text-text">{visible ? <EyeSlashIcon className="size-4" /> : <EyeIcon className="size-4" />}{visible ? m.hide : m.show}</button></div>
    {error && <p role="alert" className="text-caption">{error}</p>}
    {saved && <p role="status" className="flex items-center gap-2 text-caption"><CheckIcon className="size-4 shrink-0" />{m.saved}</p>}
    <button type="submit" disabled={busy || !current || !next || !confirm} className="flex items-center gap-2 rounded-md border border-border bg-bg-active px-4 py-2 text-body hover:bg-bg-hover disabled:opacity-40">{busy && <ArrowPathIcon className="size-4 animate-spin" />}{busy ? m.saving : m.save}</button>
  </form>;
}
