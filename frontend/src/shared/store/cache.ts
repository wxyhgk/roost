...
export function saveWorkspaceCache(state: WorkspaceSnapshot, storage: () => Pick<Storage, 'setItem'> = () => localStorage): boolean {
  try {
    const { sessions, projects, selectedId, expandedProjectIds, pinnedSessionIds, sessionSeq, projectSeq } = state;
    storage().setItem(STORAGE_KEY, JSON.stringify({ sessions, projects, selectedId, expandedProjectIds, pinnedSessionIds, sessionSeq, projectSeq }));
    return true;
  } catch { return false; }
}
