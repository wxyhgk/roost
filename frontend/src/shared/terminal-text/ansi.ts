/*
  ANSI 转义序列 → 可渲染的带样式文本行。

  **逐字抄自 deepseek-harness**（MIT，Copyright (c) 2026 DeepSeek，
  deepseek-ai/deepseek-harness，`packages/client/ui-primitives/src/ansi.ts`，
  检出提交 0d1f500）。只改了一处：颜色映射换成我们自己的主题令牌，见下面
  TOKEN_BY_BASIC_RGB。

  为什么抄而不是自己写：这个文件里是光标回放（`\r` 覆写、退格、erase-in-line）、
  制表位推进、零宽组合符和双宽字符的列数计算。这些每一条都是终端行为的细节，
  自己重写只会得到一个少了几条分支的版本——而少的那几条正是「偶尔画错一行」的来源。
  配套测试（tests/ansi.client.spec.ts）也一起抄了，移植正确性有钉子。

  我们现在把命令输出当纯文本画，`npm test` 的红绿、`git diff` 的颜色全是灰的。
*/
...
const TOKEN_BY_BASIC_RGB: Record<string, string> = {
  // 黑和白都落到正文色：终端的「黑字」在深色主题上就是看不见的字。
  '0,0,0': 'var(--color-text)',
  '255,255,255': 'var(--color-text)',
  '85,85,85': 'var(--color-text-dim)',
  '187,0,0': 'var(--color-danger)',
  '255,85,85': 'var(--color-danger)',
  '0,187,0': 'var(--color-success)',
  '0,255,0': 'var(--color-success)',
  '187,187,0': 'var(--color-warning)',
  '255,255,85': 'var(--color-warning)',
  '0,0,187': 'var(--color-accent)',
  '85,85,255': 'var(--color-accent)',
}
