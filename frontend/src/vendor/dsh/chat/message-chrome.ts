/*
  逐字抄自 deepseek-harness（MIT，`packages/client/ui-chat/src/client/chat/message-chrome.ts`，
  提交 0d1f500）。ROOST-CHANGE：只有开头那两个类型——上游拿 `@deepseek-ai/dsh-client-ui-slots`
  里的泛型 `Translate<K>` 去切他们的字典键，那是 slot 运行时的 locale seat，我们没有。
  换成同形状的本地函数类型之后，下面五个函数的函数体一行未动。
*/

/** 日期模板那一半：同年只给月日，跨年给年月日。由调用方从我们的 i18n 里取。 */
export type ClockTranslate = (
  key: 'clock.md' | 'clock.ymd',
  params: { y: number; m: number; d: number },
) => string

/** 耗时模板那一半。秒/分秒/时分秒三档，小单位已经补过零，所以可能是字符串。 */
export type RunDurationTranslate = (
  key: 'duration.seconds' | 'duration.minutes' | 'duration.hours',
  params: { hours?: number; minutes?: number | string; seconds: number | string },
) => string
