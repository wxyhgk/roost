...
import { segments, type History, type Point } from './history';
...
function Details({ snapshot: s, tab, history, onTab }: { snapshot: ServerSnapshot; tab: Tab; history: History; onTab: (tab: Tab) => void }) {
  ...
  if (tab === 'processes') return <MetricSection metric={s.processes}>{p => {
    const rows = rankProcesses(p.list, filter, sort);
...
export default function ServerMonitorView({ active, target }: { active: boolean; target?: MonitorTarget }) {
  const m = t.serverMonitor;
  const { detail: snapshot, failed: error, history } = useServerMonitor('detail', active);
  const [tab, setTab] = useState<Tab>(target?.tab ?? 'overview');
  ...
  const save = async () => {
    const { units, invalid } = parseServiceUnits(draft);
    if (invalid) { setSaveMessage(m.serviceInvalid); return; }
