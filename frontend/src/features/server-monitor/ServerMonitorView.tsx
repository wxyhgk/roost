...
export default function ServerMonitorView({ active, target }: { active: boolean; target?: MonitorTarget }) {
  ...
  const content = <div className="server-monitor flex min-h-0 flex-1 flex-col text-text">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 @container">
      ...
      {snapshot && tab === 'services' && <div className="mb-3 space-y-3">{snapshot.serviceManager === 'unsupported' && <p className="text-xs text-text-dim">{m.noServiceManager}</p>}{!editing ? <button className={button} title={m.editServices} aria-label={m.editServices} onClick={() => { setDraft(snapshot.configuredServices.join('\n')); setEditing(true); setSaveMessage(''); }}><Cog6ToothIcon className="size-4" /></button> : <div className={box}><p className="mb-2 text-xs leading-relaxed text-text-dim">{m.serviceHint[manager]}</p><textarea aria-label={m.editServices} placeholder={m.servicePlaceholder[manager]} rows={6} value={draft} onChange={e => setDraft(e.target.value)} disabled={saving} className="w-full resize-y rounded-md border border-border bg-bg p-2 font-mono text-xs" /><div className="mt-2 flex gap-2"><button className={button} title={saving ? m.saving : m.save} aria-label={saving ? m.saving : m.save} disabled={saving} onClick={() => void save()}>{saving ? <ArrowPathIcon className="size-4 animate-spin" /> : <CheckIcon className="size-4" />}</button><button className={button} title={m.cancel} aria-label={m.cancel} disabled={saving} onClick={() => { setEditing(false); setSaveMessage(''); }}><XMarkIcon className="size-4" /></button></div></div>}{saveMessage && <p role="status" className="text-xs text-text-dim">{saveMessage}</p>}</div>}
