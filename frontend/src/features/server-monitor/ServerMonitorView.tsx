...
const button = 'rounded-md border border-border p-1.5 text-xs text-text-dim hover:bg-bg-hover hover:text-text disabled:opacity-40';
...
function Stat({ icon, label, children, className = '' }: { icon: Icon; label: string; children: ReactNode; className?: string }) {
  return <div title={label} className={`flex min-w-0 items-center gap-2 text-xs tabular-nums ${className}`}><Glyph icon={icon} label={label} className="text-text-dim" /><span className="min-w-0 break-all">{children}</span></div>;
}
/*
  值本身说不出自己是什么的格子用这个。
...
function LabeledStat({ icon, label, short, children }: { icon: Icon; label: string; short: string; children: ReactNode }) {
  return <div title={label} className="flex min-w-0 flex-col gap-0.5">
    ...
    <span className="min-w-0 break-all text-xs tabular-nums">{children}</span>
  </div>;
}
...
function MetricSection<T>({ metric, children }: { metric: Metric<T>; children: (data: T) => ReactNode }) {
  return <>{metric.status !== 'ok' && <div role="status" className="mb-2 flex items-center gap-2 text-xs text-text-dim"><Health metric={metric} />{metric.data === null ? t.serverMonitor.unavailable : t.serverMonitor.stale}</div>}{metric.data !== null && children(metric.data)}</>;
}
...
function DiskCard({ disk: d, detail = false }: { disk: DiskInfo; detail?: boolean }) {
  ...
  return <div className={box}>
    <div className="flex items-center justify-between gap-2"><Stat icon={ServerStackIcon} label={m.disks}>{d.mount}</Stat><span className="shrink-0 text-xs tabular-nums">{percentage(d.usage)}</span></div>
    {detail && <p className="mt-2 break-all text-[11px] text-text-dim">{d.device} · {d.type} · {d.writable == null ? '—' : d.writable ? 'R/W' : 'R/O'}</p>}
    <Bar value={d.usage} />
    <div title={`${m.used} / ${m.total}`} className="mt-2 text-[11px] tabular-nums text-text-dim">{bytes(d.used)} / {bytes(d.size)}</div>
...
function TrendCard({ icon, label, value, detail, series, percent, metric, onClick }: { icon: Icon; label: string; value: string; detail?: ReactNode; series: Point[]; percent?: boolean; metric: Metric<unknown>; onClick?: () => void }) {
  const body = <>
    ...
    {detail && <div className="mt-1 text-[10px] tabular-nums text-text-dim">{detail}</div>}
...
function Details({ snapshot: s, tab, history, onTab }: { snapshot: ServerSnapshot; tab: Tab; history: History; onTab: (tab: Tab) => void }) {
  ...
  if (tab === 'overview') {
    ...
    return <div className="space-y-3">
      ...
      <div className="grid gap-3 @xl:grid-cols-2">
        ...
        {s.processes.data && <div className={`${box} space-y-2`}><button className="mb-1 flex w-full items-center justify-between text-xs" onClick={() => onTab('processes')} title={m.processes}><Glyph icon={QueueListIcon} label={m.topProcesses} /><span className="text-text-dim">CPU / RSS</span></button>{s.processes.data.list.slice(0, 5).map(p => <div className="flex min-w-0 items-center gap-2 text-[11px]" key={p.pid}><span className="min-w-0 flex-1 truncate" title={`${p.name} · PID ${p.pid}`}>{p.name}</span><span className="tabular-nums">{percentage(p.cpu)}</span><span className="w-16 text-right tabular-nums text-text-dim">{bytes(p.memory)}</span></div>)}</div>}
        {s.serviceManager === 'systemd' && <div className={`${box} space-y-3`}><button title={m.services} aria-label={m.services} onClick={() => onTab('services')}><Cog6ToothIcon className="size-4" /></button>{s.services.data?.slice(0, 5).map(service => <div className="flex min-w-0 items-center gap-2 text-[11px]" key={service.unit}><Glyph icon={service.active === 'active' ? CheckIcon : service.active === 'failed' ? ExclamationTriangleIcon : StopIcon} label={serviceState(service)} /><span className="min-w-0 flex-1 truncate" title={service.unit}>{service.unit.replace(/\.service$/, '')}</span><span className="text-text-dim">{serviceState(service)}</span></div>)}</div>}
      </div>
    </div>;
  }
  if (tab === 'cpu') return <MetricSection metric={s.cpu}>{c => <div className="space-y-3">
    <div className={box}>
      ...
      <div className="my-4 flex flex-wrap items-baseline gap-x-4 gap-y-1"><span className="text-3xl font-semibold tabular-nums" title={m.cpuUsage}>{percentage(c.usage)}</span><span className="text-xs tabular-nums text-text-dim" title={m.logical}>{c.busyCores == null ? '—' : c.busyCores.toFixed(2)} / {c.logicalCores}</span></div>
    ...
    <div className="grid grid-cols-2 gap-2 @xl:grid-cols-4">{[{ label: m.userCpu, value: c.user }, { label: m.systemCpu, value: c.system }, { label: m.idleCpu, value: c.idle }, { label: m.stealCpu, value: c.steal }].map(item => <div key={item.label} className={box}><p className="text-[10px] text-text-dim">{item.label}</p><p className="mt-1 text-sm tabular-nums">{percentage(item.value)}</p><Bar value={item.value} /></div>)}</div>
  ...
  if (tab === 'gpu') return <MetricSection metric={s.gpu}>{gpus => <div className="grid gap-3 @xl:grid-cols-2">
    ...
    {gpus.map((g, i) => <div className={`${box} space-y-3`} key={i}>
      ...
      <p className="text-[11px] text-text-dim">{g.vendor} · {g.bus || '—'}</p>
  ...
  if (tab === 'network') return <div className="space-y-3">
    ...
    {s.connections?.data && <div className={`${box} space-y-2`}><div className="flex items-center justify-between"><Stat icon={SignalIcon} label={m.listenPorts}>{s.connections.data.listeners.length} / {s.connections.data.listening}</Stat>{s.connections.data.limited && <Glyph icon={InformationCircleIcon} label={m.portLimit} />}</div><div className="divide-y divide-border/50">{s.connections.data.listeners.map((l, i) => <div key={i} className="flex min-w-0 items-start justify-between gap-2 py-2 text-[11px]"><div className="min-w-0"><p className="break-all font-mono">{l.address.includes(':') ? `[${l.address}]` : l.address}:{l.port}</p><p className="text-text-dim">{l.protocol}</p></div><div className="min-w-0 text-right"><p className="break-all">{l.process || '—'}</p><p className="text-text-dim">{l.pid ?? '—'}</p></div></div>)}</div></div>}
  </div>;
  ...
  return <MetricSection metric={s.services}>{services => <div className="grid gap-3 @xl:grid-cols-2">
    {!services.length && <p className="text-sm text-text-dim">{m.noServices}</p>}
    {services.map(service => <div key={service.unit} className={`${box} space-y-3`}>
      <div className="flex items-start gap-2"><Glyph icon={Cog6ToothIcon} label={service.description || m.services} className="text-text-dim" /><p title={service.description} className="min-w-0 break-all text-xs font-medium">{service.unit}</p></div>
      <div className="flex flex-wrap items-center gap-2 text-[11px]"><Glyph icon={service.active === 'active' ? CheckIcon : service.active === 'failed' ? ExclamationTriangleIcon : StopIcon} label={serviceState(service)} /><span title={`${service.active} · ${service.sub}`}>{serviceState(service)}</span><span className="ml-auto text-text-dim" title={m.bootService}>{service.enabled}</span></div>
...
export default function ServerMonitorView({ active, target }: { active: boolean; target?: MonitorTarget }) {
  ...
  const content = <div className="server-monitor flex min-h-0 flex-1 flex-col text-text">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 @container">
      {error && <div role="status" className="mb-3 flex items-center gap-2 text-xs text-text"><span>{m.failed}</span><button className={button} onClick={() => refreshServerMonitor()}>{m.retry}</button></div>}
      {!snapshot && !error && <p role="status" className="text-xs text-text-dim">{m.loading}</p>}
      {snapshot && tab === 'services' && <div className="mb-3 space-y-3">{snapshot.serviceManager === 'unsupported' && <p className="text-xs text-text-dim">{m.noSystemd}</p>}{!editing ? <button className={button} title={m.editServices} aria-label={m.editServices} onClick={() => { setDraft(snapshot.configuredServices.join('\n')); setEditing(true); setSaveMessage(''); }}><Cog6ToothIcon className="size-4" /></button> : <div className={box}><p className="mb-2 text-xs leading-relaxed text-text-dim">{m.serviceHint}</p><textarea aria-label={m.editServices} placeholder={m.servicePlaceholder} rows={6} value={draft} onChange={e => setDraft(e.target.value)} disabled={saving} className="w-full resize-y rounded-md border border-border bg-bg p-2 font-mono text-xs" /><div className="mt-2 flex gap-2"><button className={button} title={saving ? m.saving : m.save} aria-label={saving ? m.saving : m.save} disabled={saving} onClick={() => void save()}>{saving ? <ArrowPathIcon className="size-4 animate-spin" /> : <CheckIcon className="size-4" />}</button><button className={button} title={m.cancel} aria-label={m.cancel} disabled={saving} onClick={() => { setEditing(false); setSaveMessage(''); }}><XMarkIcon className="size-4" /></button></div></div>}{saveMessage && <p role="status" className="text-xs text-text-dim">{saveMessage}</p>}</div>}
      {snapshot && <Details snapshot={snapshot} tab={tab} history={history} onTab={setTab} />}
      {snapshot && <p className="mt-4 text-[10px] text-text-dim">{new Date(snapshot.timestamp).toLocaleTimeString()}</p>}
    </div>
  </div>;
  return <><div ref={sideSlot} className="flex min-h-0 flex-1 flex-col" /><dialog ref={dialog} aria-label={m.title} onCancel={e => { e.preventDefault(); setExpanded(false); }} onClose={() => setExpanded(false)} className="server-monitor m-auto h-[86dvh] max-h-[960px] w-[94vw] max-w-[1080px] overflow-hidden rounded-xl border border-border bg-bg-panel p-0 text-text shadow-modal backdrop:bg-black/55"><div className="flex h-full min-h-0 flex-col"><header className="flex h-12 shrink-0 items-center justify-between border-b border-border px-4"><h2 className="text-sm font-semibold">{m.title}{snapshot ? ` · ${snapshot.host.hostname}` : ''}</h2><button className={button} aria-label={m.collapse} onClick={() => setExpanded(false)}><XMarkIcon className="size-4" /></button></header><div ref={modalSlot} className="flex min-h-0 flex-1 flex-col" /></div></dialog>{createPortal(content, contentHost)}</>;
}
