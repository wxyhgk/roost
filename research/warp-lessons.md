...
1. ~~**退掉读屏**：闸门函数 + 显式状态 + 按 agent 的发送策略表~~ —— **这条后来被一轮实测
   推翻并细化了，见 [../tasks/gui-send-gating.md](../tasks/gui-send-gating.md)**：那张策略表
   对 roost 几乎不存在（只有一行，且实测在本机可用）；真正缺的是一道从来没有过的闸
   （前台归属 `tpgid`），而且它必须排在退掉读屏**之前**。
